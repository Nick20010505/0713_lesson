# -*- coding: utf-8 -*-
"""「Python Tutorial - Intro to AI.ipynb」的副本

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1yKTjv753nFiG2rbuhgHp0lqOysmIuYUE
...
def add(name="", phone=""):
  if len(name) > 0 and len(phone) > 0:
    add_dict = {'name': name, 'phone': phone}
    print("successful !\n")
    total_list.append(add_dict)
...
def find(name = "", phone = ""):
  ...
  if len(total_list) == 0:
    ...
    for i in total_list:
      if i["name"] == name :
        print("\n the name is ", i["name"], ", and the phone is ", i["phone"])
        factor += 1
...
def print_all():
  ...
  for i in total_list:
    count += 1
    print(count, ". the name is ", i["name"], ", and the phone is ", i["phone"])

  print("total has ", count, " data")


total_list = []

find(phone = "0987654321") #還未輸入就尋找

add("Nick", "0910440904")
add("William", "0912345678")
add("Joanne", "0987654321")
add("John") #錯誤示範

print_all()

find(name = "Angle") #找尋不存在資料
find(name = "Nick")
find(phone = "0987654321")
...
def fun(lis):
  num = lis[-1] - lis[0] + 1 - len(lis)

  if num == 0:
    print("\n didn't have missing number")

  elif num == 1: #代表只有一個數字
    total = (lis[-1] + lis[0])*(lis[-1] - lis[0] + 1) / 2
    ans = total - sum(lis)
    print("\n the missing number is ", int(ans))
    ...
    for i in lis:
      ...
      if i != lis[0]+count:
        target.append(lis[0]+count)
        count += 1

    print("\n the missing number are ", target)


fun([1,2,3,5,6,7])
fun([2,3,5,6,7]) #不從1開始
fun([1,2,3,4,5,6,7]) #無缺失數字
fun([1,2,3,5,6,8,9]) #多個數字測試
